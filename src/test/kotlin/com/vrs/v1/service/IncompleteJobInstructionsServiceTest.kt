...
import com.vrs.v1.repository.IncompleteJobInstructionsRepository
...
@QuarkusTest
class IncompleteJobInstructionsServiceTest {

    @InjectMock
    lateinit var repository: IncompleteJobInstructionsRepository

    @Inject
    lateinit var service: IncompleteJobInstructionsService

    @Test
    @RunOnVertxContext
    fun testListAll(asserter: UniAsserter) {
        val items = listOf(IncompleteJobInstructions(), IncompleteJobInstructions())
    ...
    fun testFindById(asserter: UniAsserter) {
        val item = IncompleteJobInstructions()
    ...
    fun testAdd(asserter: UniAsserter) {
        val item = IncompleteJobInstructions()
        whenever(repository.persist(any<IncompleteJobInstructions>())).thenReturn(Uni.createFrom().item(item))
