...
@QuarkusTest
class StatsDrawingsServiceTest {

    @InjectMock
    lateinit var repository: StatsDrawingsRepository
    ...
    fun testListAll(asserter: UniAsserter) {
        val items = listOf(StatsDrawings(), StatsDrawings())
    ...
    fun testAdd(asserter: UniAsserter) {
        val item = StatsDrawings()
        whenever(repository.persist(any<StatsDrawings>())).thenReturn(Uni.createFrom().item(item))
