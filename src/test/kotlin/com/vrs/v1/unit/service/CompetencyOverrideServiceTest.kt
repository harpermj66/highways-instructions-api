...
@QuarkusTest
class CompetencyOverrideServiceTest {

    @InjectMock
    lateinit var repository: CompetencyOverrideRepository
    ...
    fun testListAll(asserter: UniAsserter) {
        val items = listOf(CompetencyOverride(), CompetencyOverride())
    ...
    fun testAdd(asserter: UniAsserter) {
        val item = CompetencyOverride()
        whenever(repository.persist(any<CompetencyOverride>())).thenReturn(Uni.createFrom().item(item))
