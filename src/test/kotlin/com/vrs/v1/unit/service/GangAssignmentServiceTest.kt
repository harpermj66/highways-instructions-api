...
@QuarkusTest
class GangAssignmentServiceTest {

    @InjectMock
    lateinit var repository: GangAssignmentRepository
    ...
    fun testListAll(asserter: UniAsserter) {
        val items = listOf(GangAssignment(), GangAssignment())
    ...
    fun testAdd(asserter: UniAsserter) {
        val item = GangAssignment()
        whenever(repository.persist(any<GangAssignment>())).thenReturn(Uni.createFrom().item(item))
