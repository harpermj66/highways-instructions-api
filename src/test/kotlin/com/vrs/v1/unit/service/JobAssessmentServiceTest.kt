...
class JobAssessmentServiceTest {
    ...
    @InjectMock
    lateinit var repository: JobAssessmentRepository
    ...
    fun testListAll(asserter: UniAsserter) {
        val items = listOf(JobAssessment(), JobAssessment())
    ...
    fun testAdd(asserter: UniAsserter) {
        val item = JobAssessment()
        whenever(repository.persist(any<JobAssessment>())).thenReturn(Uni.createFrom().item(item))
