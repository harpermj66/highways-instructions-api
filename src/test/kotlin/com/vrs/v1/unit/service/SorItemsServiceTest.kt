...
@QuarkusTest
class SorItemsServiceTest {

    @InjectMock
    lateinit var repository: SorItemsRepository
    ...
    fun testListAll(asserter: UniAsserter) {
        val items = listOf(SorItems(), SorItems())
    ...
    fun testAdd(asserter: UniAsserter) {
        val item = SorItems()
        whenever(repository.persist(any<SorItems>())).thenReturn(Uni.createFrom().item(item))
