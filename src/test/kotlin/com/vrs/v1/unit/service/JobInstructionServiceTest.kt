...
@QuarkusTest
class JobInstructionServiceTest {

    @InjectMock
    lateinit var repository: JobInstructionRepository
    ...
    fun testListAll(asserter: UniAsserter) {
        val items = listOf(JobInstruction(), JobInstruction())
    ...
    fun testAdd(asserter: UniAsserter) {
        val item = JobInstruction()
        whenever(repository.persist(any<JobInstruction>())).thenReturn(Uni.createFrom().item(item))
