...
class OfficersResource(private val service: OfficersService) {

    @GET
    fun listAll(): Uni<List<Officers>> = service.listAll()

    @GET
    @Path("/{id}")
    fun getById(@PathParam("id") id: Long): Uni<Officers> {
        return service.findById(id)
            .onItem().ifNull().failWith { NotFoundException("Officers not found with id: $id") }
    }

    @POST
    fun add(entity: Officers): Uni<Response> {
