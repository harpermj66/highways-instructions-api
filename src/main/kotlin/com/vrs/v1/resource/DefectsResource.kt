...
class DefectsResource(private val service: DefectsService) {

    @GET
    fun listAll(): Uni<List<Defects>> = service.listAll()

    @GET
    @Path("/{id}")
    fun getById(@PathParam("id") id: Long): Uni<Defects> {
        return service.findById(id)
            .onItem().ifNull().failWith { NotFoundException("Defects not found with id: $id") }
    }

    @POST
    fun add(entity: Defects): Uni<Response> {
