...
class PriorityResource(private val service: PriorityService) {

    @GET
    fun listAll(): Uni<List<Priority>> = service.listAll()

    @GET
    @Path("/{id}")
    fun getById(@PathParam("id") id: Long): Uni<Priority> {
        return service.findById(id)
            .onItem().ifNull().failWith { NotFoundException("Priority not found with id: $id") }
    }

    @POST
    fun add(entity: Priority): Uni<Response> {
